...
class shopProductfeaturesPluginSaveController extends waJsonController
{
    ...
    public function execute()
    {
        ...
        $this->checbox_add = $plugin->getSettings('checkbox_add');
        ...
        $rows = $type_features_model->getByField('feature_id', array_merge(array_keys($this->features_id_code), array_keys($children_codes)), true);
            ...
            if ($p['type_id']) {
                foreach ($this->type_features[$p['type_id']] as $code) {
                    if (isset($this->values[$code]) && isset($this->features[$code])) {
                        ...
                        if (empty($this->features[$code]['multiple']) || !$this->checbox_add) {
