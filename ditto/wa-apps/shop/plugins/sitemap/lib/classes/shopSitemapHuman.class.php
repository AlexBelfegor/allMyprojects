...
class shopSitemapHuman extends waSitemapConfig
{
    protected $app_id;
    ...
    public function execute($n = 1)
    {
        ...
        foreach ($routes as $route) {
            ...
            if ($n == 1) {
                // categories
                $sql = "SELECT c.id,c.name,c.parent_id,c.left_key,c.url,c.full_url,c.create_datetime,c.edit_datetime
                ...
                foreach ($categories as $c_id => $c) {
                    ...
                    $this->data['Категории'][] = $this->addUrl(str_replace('%CATEGORY_URL%', $url, $category_url),$c['edit_datetime'] ? $c['edit_datetime'] : $c['create_datetime'], self::CHANGE_WEEKLY, 0.6,"",$c['name']);
                }

                $main_url = $this->getUrl('');
                // pages
                $sql = "SELECT full_url, name, url, create_datetime, update_datetime FROM ".$page_model->getTableName().'
                ...
                foreach ($pages as $p) {
                    $this->data['Страницы'][] = $this->addUrl($main_url.$p['full_url'], $p['update_datetime'] ? $p['update_datetime'] : $p['create_datetime'], self::CHANGE_MONTHLY, 0.6, "", $p['name']);
                }
                ...
                if ($plugin_urls) {
                    foreach ($plugin_urls as $urls) {
                        foreach ($urls as $url) {
                            $this->data['Плагины'][] = $this->addUrl($url['loc'], ifset($url['lastmod'], time()), ifset($url['changefreq']), ifset($url['priority']));
                        }
                    }
                }

                // main page
                //$this->data[] = $this->addUrl($main_url, time(), self::CHANGE_DAILY, 1);
            }
            ...
            foreach ($products as $p) {
                ...
                $this->data['Продукты'][] = $this->addUrl($url, $p['edit_datetime'] ? $p['edit_datetime'] : $p['create_datetime'], self::CHANGE_MONTHLY, 0.8, $img, $p['name']);
            }
    ...
    public function addUrl($loc, $lastmod, $changefreq = null, $priority = null, $img = null, $name = null)
    {
        $data['name'] = htmlspecialchars($name, ENT_NOQUOTES);
        $data['link'] = htmlspecialchars($loc, ENT_NOQUOTES);
    ...
    public function blog_execute()
    {
        ...
        foreach ($routes as $route) {
            ...
            foreach ($blogs as $blog_id => $blog) {
                if (!$default_blog_id || ($blog_id == $default_blog_id) ) {
                    ...
                    foreach ($posts as $post) {
                        ...
                        $this->data['Блог'][] = $this->addUrl($post['link'], $post_lastmod, "", "", "", $post['title']);
                    }
                }
            }
            ...
            $pages = $page_model->query($sql, array('domain' => $domain, 'route' => $route['url']))->fetchAll();
            foreach ($pages as $p) {
                $this->data[] = $this->addUrl($main_url.$p['full_url'], $p['update_datetime'] ? $p['update_datetime'] : $p['create_datetime'], self::CHANGE_MONTHLY, 0.6);
            }


            $this->data[] = $this->addUrl(wa()->getRouteUrl($app_id."/frontend", array(), true), $lastmod, self::CHANGE_DAILY, 1.0);*/
        }
    }
}
