...
class shopBwyandexmarketPluginRunController extends waLongActionController
{
    ...
    protected function init()
    {
        try {
            ...
            $profiles = new shopImportexportHelper('bwyandexmarket');
...
XML;
            waFiles::copy(shopBwyandexmarketPlugin::path('shops.dtd'), $this->getTempPath('shops.dtd'));
            ...
            $this->data['path'] = array(
                'offers' => shopBwyandexmarketPlugin::path($profile_id.'.xml'),
            );
    ...
    private function validate()
    {
        ...
        shopBwyandexmarketPlugin::path('shops.dtd');
    ...
    private function getTempPath($file = null)
    {
        ...
        return wa()->getTempPath('plugins/bwyandexmarket/', 'shop').$file;
    }
        ...
        while ((--$chunk > 0) && ($product = reset($products))) {
            ...
            if ($check_type && $check_price && $check_category) {
                if (!empty($this->data['export']['sku'])) {
                    ...
                    $skus = $this->getDataByProductIdFull($product['id']);
    ...
    //get list features
    //BW
    public function GetListFeatures($sku_id)
    {
        ...
        SELECT
        	...
        	f.type
        FROM
        	".$product_features_model->getTableName()." as prod_f,
        	shop_feature as f
        where
        	prod_f.sku_id = ".$sku_id." and
        	prod_f.feature_id = f.id
        ";

        return $product_features_model->query($sql)->fetchAll();
    }

    //BW
    //get getDataByProductIdFull
    //BW
    public function getDataByProductIdFull($product_id, $fill_empty_sku_by_null = false)
    {
        ...
        $sku_model = new shopProductSkusModel();
        $skus = $sku_model->getByField('product_id', $product_id, true);

        foreach ($skus as &$sku) {
            $sku['list_features'] = $this->GetListFeatures($sku['id']);
    ...
    //BW
    public function getFeaturesProductAvailable($id = 0)
    {
        ...
        foreach ($offer_map as $field => $info)
        {
            ...
            if (!empty($info['source']) && (!ifempty($info['category'], array()) || in_array('simple', $info['category']))) {
                                ...
                                case 'id':
                                    if (!empty($sku['id']) && ($sku['id'] != $product['sku_id'])) {
				                        ...
				                        /*if (!isset($product['features'])) {
								            ...
								            $feature_model_2 = new shopFeatureModel();
								            $product['features_full'] = $feature_model_2->getByCode($feature_codes);

							                //selected
								            $product['features_selectable'] = $this->getFeaturesProductAvailable($product['id']);
				                        }*/
                ...
                if (!in_array($value, array(null, false, ''), true)) {
                    ...
                    print_r($product_xml);
                    print_r($field);*/
                    $this->addDomValue($product_xml, $field, $value, !empty($info['attribute']), $sku);
                }
            }
        }
     ...
     */
    private function addDomValue(&$dom, $field, $value, $is_attribute = false, $sku = null)
    {
        /*BW*/
        if($field == "oldprice" && $value == "0.00")	return;
        ...
        if (is_array($value)) {
            ...
            else
            {
                ...
                {
		            /*BW*/
		            if ($field == 'param' || trim($value_item) == 'param')
		            {
			            //$this->_log($value_item);
                        $n_ame = key($value_item);
		                $v_lue = current($value_item);

			            if($n_ame == "") continue;

			            $child = $this->dom->createElement($field, trim($v_lue));
		                $child->setAttribute('name', $n_ame);

		                if(isset($value_item['unit']))
		                {
		                	$child->setAttribute('unit', $value_item['unit']);
		                }
		            ...
		            {
			            $child = $this->dom->createElement($field, trim($value_item));
			            $dom->appendChild($child);
		            }
                }
            }
    ...
    private function format($field, $value, $info = array(), $data = array(), $sku_data = null)
    {
        ...
        switch ($field) {
            case 'param':
                ...
                /*$this->_log($data['features'],"log2");
                $this->_log($data['features_full'],"log3");
                $this->_log($data['features_selectable'],"log4");
                $this->_log($sku_data,"log5");*/

                $fucking_param = true;
                $values = array();

                $i = 0;
                foreach($data['features'] as $f_code => $f_value)
                {
               		if (!is_array($f_value))
                	{
                		if($f_code == "youtube" || $f_code == "")	continue;

               			if($f_code == "color")
               			{
			               	$f_value = preg_replace('/<br\/?\s*>/', "\n", $f_value);
			                $f_value = preg_replace("/[\r\n]+/", "\n", $f_value);
			                $f_value = strip_tags($f_value);
               			}

                		$values[$i][$data['features_full'][$f_code]['name']] = $f_value;
                ...
                foreach($sku_data['list_features'] as $f_value)
                {
                	$code = $f_value['code'];

                	if($code == "youtube" || $code == "")	continue;

                	if(isset($data['features_selectable'][$code]['values'][$f_value['feature_value_id']]['value']))
                	{
                    	if($data['features_selectable'][$code]['values'][$f_value['feature_value_id']]['selected'])
                    	{
                    		$value = @$data['features_selectable'][$code]['values'][$f_value['feature_value_id']]['value'];
                    		...
                    		$values[$i][$data['features_selectable'][$code]['name']] = $value;
                	...
                	elseif($code == "size" && $sku_data['name'])
                	{
                    	$values[$i]["Размер"] = $sku_data['name'];
                    	$values[$i]["unit"] = "UA";
            ...
            case 'oldprice':
               	...
               	if ($sku_data) {
                    if (!in_array($data['currency'], $this->data['currency'])) {
                        ...
                        $value = $currency_model->convert($data['compare_price_selectable'], $data['currency'], $this->data['primary_currency']);
                        $data['currency'] = $this->data['primary_currency'];
                    }
                } else {
                    if (!in_array($data['currency'], $this->data['currency'])) {
                        ...
                        $value = $currency_model->convert($data['compare_price_selectable'], $this->data['primary_currency'], $data['currency']);
                        $data['currency'] = $this->data['primary_currency'];
                    } elseif ($this->data['primary_currency'] != $data['currency']) {
                        if (!$currency_model) {
                            $currency_model = new shopCurrencyModel();
            ...
            case 'picture':
				...
				$first_img = !empty($this->data['export']['first_img']);
    ...
    private function error($message)
    {
        ...
        waFiles::create($path.'/shop/plugins/bwyandexmarket.log');
        waLog::log($message, 'shop/plugins/bwyandexmarket.log');
    }
    ...
    private function _log($data, $file = "log")
    {
        $path = wa()->getConfig()->getPath('log');

        $f= fopen($path."/shop/plugins/".$file.".log","a+");

        if(is_array($data))
		{
			foreach($data as $key => $val)
			{
				$dd .= $key."=".var_export($val,true)."\n";
			}
        }
        else
        {
