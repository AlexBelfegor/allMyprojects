 ...
 */
class shopBwyandexmarketPlugin extends shopPlugin
{
    private $types;
    ...
    public function backendProductsEvent()
    {
        ...
        $html = <<<HTML
 <li data-action="export" data-plugin="bwyandexmarket" title="Экспорт выбранных товаров в Яндекс.Маркет">
...
HTML;
    ...
    {
        $path = wa()->getDataPath('plugins/bwyandexmarket/'.$file, false, 'shop', true);
        if ($file == 'shops.dtd') {
            if (!file_exists($path)) {
