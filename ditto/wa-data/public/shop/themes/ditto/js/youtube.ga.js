...
function onPlayerProgressChange() {
    ...
    if (progress) {
        ga('send', 'event', 'YouTube', 'click', 'Played video:' + quality, YT_GA.url, undefined, true);
    }
}
...
function onPlayerPlaybackQualityChange(event) {
    ...
    if (quality) {
        ga('send', 'event', 'YouTube', 'click', 'Video quality:' + quality, YT_GA.url, undefined, true);
    }
}
...
function onPlayerStateChange(event) {
    ...
    if (event.data === YT.PlayerState.PLAYING && !YT_GA.videoPlayed) {
        ...
        ga('send', 'event', 'YouTube', 'click', 'Paused video:' + YT_GA.url, undefined, true);
        YT_GA.videoPaused = true; // Avoid subsequent pause trackings

    } else if (event.data === YT.PlayerState.ENDED && !YT_GA.videoCompleted) {
        ga('send', 'event', 'YouTube', 'click', 'Completed video:' + YT_GA.url, undefined, true);
